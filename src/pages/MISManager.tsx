...
import { Database, Plus, Users, FileText, Settings, Building, GraduationCap, Heart, Briefcase, ShoppingCart, Hotel, DollarSign, Download, Power, Check, X } from 'lucide-react';
...
const defaultSubModules = [
  ...
  { name: 'Database Connector', status: 'online' },
  { name: 'Automated Reports', status: 'online' },
  { name: 'Cron Sync Automation', status: 'offline' },
  { name: 'AI Optimization Assistant', status: 'online' },
  { name: 'Omni Messaging Router', status: 'online' },
  { name: 'System Notifications Logs', status: 'online' },
];
...
const defaultAutomations = [
  { id: 1, name: 'Auto-send daily attendance reports to parents', active: true },
  { id: 2, name: 'Flag students missing 3+ consecutive days', active: true },
  { id: 3, name: 'Generate weekly financial summary for admin', active: false },
  { id: 4, name: 'Database backup every 12 hours', active: true },
];
...
export default function MISManager() {
  ...
  const [systems, setSystems] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('kanyoza_mis_systems');
  ...
  // Automations Switch State
  const [automations, setAutomations] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('kanyoza_mis_automations');
      return saved ? JSON.parse(saved) : defaultAutomations;
    } catch {
  ...
  // Settings State
  const [adminEmail, setAdminEmail] = useState('admin@kanyoza.edu');
  const [maintenanceMode, setMaintenanceMode] = useState('Disabled (Normal Operation)');

  useEffect(() => {
    localStorage.setItem('kanyoza_mis_systems', JSON.stringify(systems));
  }, [systems]);

  useEffect(() => {
    localStorage.setItem('kanyoza_mis_automations', JSON.stringify(automations));
  }, [automations]);

  const selectedDetails = misTypesList.find(t => t.id === selectedType) || misTypesList[0];
  const SelectedIcon = selectedDetails.icon;

  const handleAddMIS = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newName.trim() || !newOrg.trim()) {
      triggerNotification({
        title: 'Validation Error',
        message: 'MIS name and organization are required.',
        type: 'warning',
      });
      return;
    }
    ...
    const newSystem = {
      name: newName.trim(),
      type: selectedDetails.name,
      org: newOrg.trim(),
      users: parseInt(newUsers) || 1,
      status: 'Active',
      activity: 'Just now',
    };
    ...
    triggerNotification({
      title: 'MIS Initialized',
      message: `"${newSystem.name}" is now fully integrated on core services.`,
      type: 'success',
    });
  };

  const handleToggleSystemStatus = (idx: number, name: string, currentStatus: string) => {
    const nextStatus = currentStatus === 'Active' ? 'Inactive' : 'Active';
    setSystems((prev) =>
      prev.map((s, i) => (i === idx ? { ...s, status: nextStatus } : s))
    );
    triggerNotification({
      title: 'Status Altered',
      message: `"${name}" set to ${nextStatus.toUpperCase()}`,
      type: 'info',
    });
  };
  ...
  const handleDownloadReport = (name: string) => {
    triggerNotification({
      ...
      message: `Export package for "${name}" has been transferred.`,
      type: 'success',
    });
  };

  const handleToggleAutomation = (id: number, name: string) => {
    setAutomations((prev) =>
      prev.map((a) => (a.id === id ? { ...a, active: !a.active } : a))
    );
    const item = automations.find((a) => a.id === id);
    const isNowActive = item ? !item.active : false;
    triggerNotification({
      title: 'Cron Trigger Shifted',
      message: `"${name}" is now ${isNowActive ? 'ENABLED' : 'DISABLED'}`,
      type: 'info',
    });
  };
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    triggerNotification({
      ...
      message: `Operational values for "${selectedDetails.name}" saved.`,
      type: 'success',
    });
  };
  ...
  return (
    <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-8 pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <button
          className="bg-brand-primary text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-brand-primary/90 shadow-glow-primary transition-all"
          onClick={() => setShowAddForm(!showAddForm)}
        >
           <Plus className="w-5 h-5" /> {showAddForm ? 'Cancel Creation' : 'Add MIS'}
        </button>
      </div>
      ...
      <AnimatePresence>
        {showAddForm && (
          <motion.form
            ...
            onSubmit={handleAddMIS}
            className="p-5 bg-brand-surface border border-brand-border rounded-2xl space-y-4 overflow-hidden"
          >
            <h3 className="text-sm font-mono font-bold uppercase tracking-wider text-brand-text">Instantiate Management Information System</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-brand-text-muted mb-1">System Custom Name</label>
                <input
                  type="text"
                  placeholder="e.g. Blantyre Academy MIS"
                  value={newName}
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-brand-text-muted mb-1">Organization Name</label>
                <input
                  type="text"
                  placeholder="e.g. Blantyre Private Academy"
                  value={newOrg}
              ...
              <div>
                <label className="block text-[10px] font-mono font-bold uppercase text-brand-text-muted mb-1">Initial User Allotment</label>
                <input
                  type="number"
                  value={newUsers}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
         <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
            ...
            <div className="text-3xl font-bold text-brand-text">{systems.length}</div>
         </div>
         <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
            ...
            <div className="text-3xl font-bold text-brand-text">
              {systems.reduce((acc, curr) => acc + curr.users, 0).toLocaleString()}
            </div>
         </div>
         <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
        {misTypesList.map(type => (
          <button
            ...
            className={cn(
              "min-w-[150px] p-5 rounded-2xl border flex flex-col items-center justify-center gap-4 transition-all flex-shrink-0",
              selectedType === type.id 
                ? "bg-brand-primary/10 border-brand-primary text-brand-primary shadow-glow-primary" 
                : "bg-brand-surface border-brand-border text-brand-text hover:bg-brand-elevated"
            )}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 bg-brand-surface border border-brand-border rounded-2xl p-6 flex flex-col">
            ...
            <h3 className="text-sm font-bold uppercase tracking-widest text-brand-text-muted mb-4 font-mono">Sub-modules Status</h3>
            <div className="space-y-3 flex-1 overflow-y-auto pr-2 scrollbar-hide max-h-[350px]">
              {defaultSubModules.map(mod => (
           ...
           <div className="p-8 flex-1 bg-brand-surface overflow-y-auto">
             ...
             {activeTab === 'Reports' && (
               <div className="space-y-4">
                 {[
                   {name:'Monthly Attendance Report', date:'Oct 24, 2026', type:'PDF'},
                   {name:'Financial Summary Q3', date:'Oct 20, 2026', type:'XLSX'},
                   {name:'Grade Distribution Analytics', date:'Oct 15, 2026', type:'PDF'},
                   {name:'Teacher Performance Metrics', date:'Oct 10, 2026', type:'CSV'},
                   {name:'Term 3 Student Results', date:'Oct 01, 2026', type:'PDF'},
                 ].map((r,i) => (
                   <div key={i} className="flex items-center justify-between p-4 bg-brand-elevated border border-brand-border rounded-xl hover:border-brand-primary/30 transition-colors">
                     ...
                     <button
                       className="p-2.5 bg-brand-surface hover:bg-brand-primary/20 hover:text-brand-primary rounded-lg text-brand-text-muted border border-brand-border transition-colors"
             ...
             {activeTab === 'Automation' && (
               <div className="space-y-4">
                 {automations.map((a) => (
                   <div key={a.id} className="flex items-center justify-between p-5 bg-brand-elevated border border-brand-border rounded-xl">
                     <span className="text-sm font-bold text-brand-text">{a.name}</span>
                     <button
                       className={cn(
                         "w-12 h-7 rounded-full flex items-center px-1 transition-all border",
                         a.active 
                           ? "bg-brand-primary border-brand-primary justify-end" 
                           : "bg-brand-surface border-brand-border justify-start"
                       )}
                       onClick={() => handleToggleAutomation(a.id, a.name)}
                     >
             ...
             {activeTab === 'Settings' && (
               <form onSubmit={handleSaveSettings} className="space-y-6 max-w-md">
                 <div>
                   ...
                   <input
                     type="text"
                     defaultValue={`Kanyoza ${selectedDetails.name}`}
                     className="w-full bg-brand-elevated border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-text outline-none focus:border-brand-primary transition-colors"
                   />
                 </div>
                 <div>
                 ...
                 <div>
                   ...
                   <select
                     ...
                     className="w-full bg-brand-elevated border border-brand-border rounded-xl px-4 py-3 text-sm text-brand-text outline-none focus:border-brand-primary transition-colors appearance-none"
                   >
                     <option value="Disabled (Normal Operation)">Disabled (Normal Operation)</option>
                     <option value="Enabled (Admins only)">Enabled (Admins only)</option>
                     <option value="Enabled (Complete Offline)">Enabled (Complete Offline)</option>
                   </select>
                 </div>
                 <button
                   type="submit"
                   className="w-full bg-brand-primary text-white font-bold text-sm py-3.5 rounded-xl shadow-glow-primary hover:bg-brand-primary/90 transition-all mt-4"
                 >
                   Save Configuration
                 </button>
               </form>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left min-w-[800px] border-collapse">
            <thead>
              <tr className="border-b border-brand-border text-xs text-brand-text-muted font-mono uppercase tracking-wider bg-brand-elevated">
            ...
            <tbody>
              {systems.map((s,i) => (
                <tr key={i} className="border-b border-brand-border/50 hover:bg-brand-elevated/40 transition-colors">
                  <td className="py-4 px-4 text-sm font-bold text-brand-text">{s.name}</td>
                  <td className="py-4 px-4 text-xs font-mono text-brand-text-muted">{s.type}</td>
                  ...
                  <td className="py-4 px-4">
                    <span className={cn(
                      ...
                      s.status === 'Active' 
                        ...
                        : "bg-brand-text-muted/15 text-brand-text-muted border-brand-border"
                    )}>
                      {s.status}
                    </span>
                  </td>
                  ...
                  <td className="py-4 px-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        className="p-2 bg-brand-elevated hover:bg-brand-border/50 rounded-lg transition-colors border border-brand-border text-brand-text-muted hover:text-brand-text"
                        onClick={() => {
                          setSelectedType(s.type.replace(' MIS', '').replace(' ', '').toLowerCase());
                          triggerNotification({
                            title: 'Active MIS Context',
                            message: `Loaded context details for "${s.name}"`,
                            type: 'info',
                          });
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={() => handleToggleSystemStatus(i, s.name, s.status)}
                      >
              ...
              {systems.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-brand-text-muted font-mono uppercase text-xs tracking-wider">
                    No registered MIS organizations.
                  </td>
                </tr>
              )}
            </tbody>

...
import { Calendar, Plus, Clock, Activity, Share2, X, Send, Trash2, CheckCircle2 } from 'lucide-react';
...
export default function Posts() {
  ...
  const [thumbnail, setThumbnail] = useState('https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=300&q=80');
  ...
  const handleCreatePost = (e: React.FormEvent) => {
    ...
    const newPost: Post = {
      ...
      thumbnail: thumbnail.trim() || 'https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&w=300&q=80',
    };

    addPost(newPost);
    setTitle('');
    setIsNewPostModalOpen(false);
    showToast(`Broadcast successfully published to ${platform.toUpperCase()}!`);
  };
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-20 right-8 z-50 bg-brand-success/90 backdrop-blur-md text-white px-5 py-3 rounded-xl shadow-2xl flex items-center space-x-2 font-mono text-xs font-bold"
          >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            ...
            >
              ...
              <div className="flex space-x-3 pt-4 border-t border-brand-border">
                <button
                  onClick={() => {
                    showToast(`Re-broadcast triggered for ${selectedPost.title}!`);
                    setSelectedPost(null);
                  }}
                  className="flex-1 py-2.5 rounded-xl bg-brand-primary text-white font-bold uppercase text-xs tracking-wider hover:bg-brand-primary/90 transition-colors flex items-center justify-center"
                >

...
export default function SocialAccounts() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (wsList.length > 0) {
        const activeId = selectedWorkspaceId && wsList.find(w => w.id === selectedWorkspaceId)
          ? selectedWorkspaceId
          : wsList[0].id;
  ...
  useEffect(() => { loadData(); }, [selectedWorkspaceId]);

  const openNew = () => {
    setEditingAccount(null);
    setModalPlatform('facebook');
    setModalAccountName('');
    setModalTimezone('Africa/Blantyre');
  ...
  const openEdit = (account: SocialAccount) => {
    ...
    for (const field of PLATFORM_FIELDS[account.platform]) {
      creds[field.key] = (account as any)[field.key] || '';
    }
  ...
  const handleToggle = async (account: SocialAccount) => {
    try {
      const payload = {
        ...
        ...Object.fromEntries(PLATFORM_FIELDS[account.platform].map(f => [f.key, (account as any)[f.key] || ''])),
      };
  ...
  const handleHealthCheck = async (account: SocialAccount) => {
    ...
    try {
      const res = await apiFetch<{ healthy?: boolean; status?: string }>(`/social-accounts/${account.id}/health-check`, { method: 'POST' });
      showToast('success', res.healthy ? 'Connection healthy' : 'Connection failed');
      loadData();
    } catch (err: any) {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      if (editingAccount?.id) {
        ...
        setTestResult({ status: 'success', message: 'Credential format validated' });
      }
    } catch (err: any) {
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className={cn(
              "fixed top-20 right-6 z-50 px-5 py-3 rounded-xl border text-sm font-bold font-mono shadow-lg",
              toast.type === 'success' ? "bg-brand-success/10 text-brand-success border-brand-success/30" : "bg-brand-danger/10 text-brand-danger border-brand-danger/30"
            )}
          >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl font-bold uppercase tracking-tight flex items-center gap-3">
            <Link className="w-8 h-8 text-brand-primary" />
            Social Accounts
            <span className="ml-2 text-xs font-mono font-bold bg-brand-primary/10 border border-brand-primary/20 text-brand-primary px-2.5 py-1 rounded-full uppercase">
              {workspaceAccounts.length} Connected
            </span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-brand-surface border border-brand-border rounded-2xl p-6 animate-pulse space-y-4">
              ...
              <div className="h-4 bg-brand-elevated rounded w-1/2" />
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start gap-4 mb-4">
                    ...
                    <span className={cn(
                      "px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider border rounded-md font-mono",
                      ...
                      (!account.health_status || account.health_status === 'unknown') && "bg-brand-border text-brand-text-muted"
                    )}>{account.health_status || 'Unknown'}</span>
                  </div>
                  ...
                  <div className="pt-6 mt-6 border-t border-brand-border/50 space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setConfirmDelete(account)} className="py-2 px-3 bg-brand-danger/10 hover:bg-brand-danger/20 text-brand-danger rounded-xl transition-all">
              ...
              <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select disabled={!!editingAccount} value={modalPlatform} onChange={e => { setModalPlatform(e.target.value as SocialAccount['platform']); setCredentials({}); setVisibleFields({}); }} className="w-full bg-brand-elevated border border-brand-border rounded-xl px-4 py-2.5 text-xs text-brand-text font-bold uppercase focus:outline-none focus:border-brand-primary cursor-pointer">
                  ...
                  <div>
                    ...
                    <input type="text" required placeholder="e.g. Kanyoza Marketing" value={modalAccountName} onChange={e => setModalAccountName(e.target.value)} className="w-full bg-brand-elevated border border-brand-border rounded-xl px-4 py-2.5 text-xs text-brand-text focus:outline-none focus:border-brand-primary" />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-mono font-bold uppercase tracking-wider text-brand-text-muted mb-1.5">Timezone</label>
                    <select value={modalTimezone} onChange={e => setModalTimezone(e.target.value)} className="w-full bg-brand-elevated border border-brand-border rounded-xl px-4 py-2.5 text-xs text-brand-text focus:outline-none focus:border-brand-primary cursor-pointer">
                  ...
                  <div className="flex flex-col justify-end">
                    <div className="flex items-center justify-between p-2.5 bg-brand-elevated border border-brand-border rounded-xl">
                      ...
                      <button type="button" onClick={() => setModalEnabled(!modalEnabled)} className={cn("w-11 h-6 rounded-full flex items-center px-1 transition-all border", modalEnabled ? "bg-brand-primary border-brand-primary justify-end" : "bg-brand-surface border-brand-border justify-start")}>
                        <motion.div layout className="w-4 h-4 bg-white rounded-full shadow" />
                      </button>
                    </div>
                  </div>
                    ...
                    {PLATFORM_FIELDS[modalPlatform].map(field => (
                      <div key={field.key} className="space-y-1.5">
                        ...
                        <div className="relative">
                          <input type={field.secret && !visibleFields[field.key] ? 'password' : 'text'} required value={credentials[field.key] || ''} onChange={e => setCredentials(prev => ({ ...prev, [field.key]: e.target.value }))} placeholder={field.placeholder} className="w-full bg-brand-elevated border border-brand-border rounded-xl pl-4 pr-12 py-2.5 text-xs text-brand-text font-mono focus:outline-none focus:border-brand-primary" />
                          {field.secret && (
                ...
                <div className="p-4 bg-brand-surface border border-brand-border rounded-xl space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="text-xs font-mono font-bold uppercase text-brand-text">Test Connection</h4>
                      <p className="text-[11px] text-brand-text-muted">Verify credentials before saving</p>
                    </div>
                  ...
                  <AnimatePresence>
                    {testResult && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className={cn("p-3 rounded-lg border text-xs font-mono flex items-start gap-2", testResult.status === 'success' ? "bg-brand-success/10 text-brand-success border-brand-success/20" : "bg-brand-danger/10 text-brand-danger border-brand-danger/20")}>
                        {testResult.status === 'success' ? <CheckCircle className="w-4 h-4 mt-0.5" /> : <XCircle className="w-4 h-4 mt-0.5" />}
                        {testResult.message}
                      </motion.div>
                    )}
                  </AnimatePresence>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-md bg-brand-surface border border-brand-border rounded-2xl p-6 shadow-2xl space-y-6">
              <div className="flex items-start gap-3">
                <div className="p-3 bg-brand-danger/10 text-brand-danger rounded-xl"><AlertTriangle className="w-6 h-6" /></div>
                <div>
                  <h3 className="text-base font-bold text-brand-text uppercase">Disconnect "{confirmDelete.account_name}"?</h3>
                  <p className="text-xs text-brand-text-muted mt-1 font-mono">Posts to this account will stop. Credentials will be discarded.</p>
                </div>
              </div>

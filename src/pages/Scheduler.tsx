...
import {
  ...
  History, GitBranch, TriangleAlert,
} from 'lucide-react';
...
import {
  ...
  pauseWorkflow, resumeWorkflow, fetchWorkflowStatus, fetchWorkflowState, runWorkflow, triggerPost,
  type ScheduleConfig,
} from '../lib/api';
...
function StatusBadge({ ok }: { ok: boolean }) {
  return (
    <span className={cn('inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase font-mono',
...
export default function Scheduler() {
  ...
  const { data: historyData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ history?: unknown[]; workflows?: unknown[] }>;
    },
  ...
  // ── Bot / workflow state ───────────────────────────────────────────────────
  const sched             = (botStatus as { scheduler?: { running?: boolean; posts_today?: number; target_posts_per_day?: number; last_post?: string; queue_size?: number } } | undefined)?.scheduler;
  const schedulerRunning  = wfStatus?.status === 'running' || !!sched?.running;
  const postsToday        = sched?.posts_today ?? 0;
  const targetPosts       = sched?.target_posts_per_day ?? displayContentHours.length;
  const lastPost          = sched?.last_post ?? null;
  const queueSize         = sched?.queue_size ?? 0;

  // Workflow definitions
  const wfList = (wfState?.workflows ?? (wfState?.workflow ? [wfState.workflow] : [])) as Array<{ id?: string | number; name?: string; status?: string; last_run?: string; enabled?: boolean }>;

  const workflowHistory = (historyData?.history ?? (historyData as { workflows?: unknown[] })?.workflows ?? []) as Array<{ id?: string | number; type?: string; workflow_type?: string; status?: string; started_at?: string; created_at?: string; duration_ms?: number; duration?: string }>;

  // ── Mutations ──────────────────────────────────────────────────────────────
  const saveMut = useMutation({
    mutationFn: () => updateSchedule(cfg, { content_hours_utc: displayContentHours, news_hours_utc: displayNewsHours }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['schedule'] }); setContentHours(null); setNewsHours(null); showToast('Schedule saved and applied.'); },
  ...
  const postMut = useMutation({
    mutationFn: async (): Promise<BotPostResult> => {
      const base = restEndpoint.replace(/\/+$/, '');
      const res = await fetch(`${base}/bot/post`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${masterToken}` } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return res.json() as Promise<BotPostResult>;
    },
    onSuccess: (d: BotPostResult) => { qc.invalidateQueries({ queryKey: ['bot-status'] }); showToast(d?.queued ? 'Post queued for publish.' : 'Queue full — try again shortly.', d?.queued ? 'success' : 'error'); },
  ...
  const newsMut = useMutation({
    mutationFn: async (): Promise<BotActionResult> => {
      const base = restEndpoint.replace(/\/+$/, '');
      const res = await fetch(`${base}/bot/news`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${masterToken}` } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const engageMut = useMutation({
    mutationFn: async (): Promise<BotActionResult> => {
      const base = restEndpoint.replace(/\/+$/, '');
      const res = await fetch(`${base}/bot/engage`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${masterToken}` } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      return res.json() as Promise<BotActionResult>;
    },
  ...
  const pauseMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['bot-status'] }); qc.invalidateQueries({ queryKey: ['workflow-status'] }); showToast('Workflow paused.'); },
    onError: (err: Error) => showToast(err?.message || 'Failed to pause workflow.', 'error'),
  });
  const resumeMut = useMutation({
    mutationFn: () => resumeWorkflow(cfg),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['bot-status'] }); qc.invalidateQueries({ queryKey: ['workflow-status'] }); showToast('Workflow resumed.'); },
    onError: (err: Error) => showToast(err?.message || 'Failed to resume workflow.', 'error'),
  });
  const runMut = useMutation({
    mutationFn: () => runWorkflow(cfg),
    onSuccess: (res) => { qc.invalidateQueries({ queryKey: ['workflow-history'] }); qc.invalidateQueries({ queryKey: ['workflow-state'] }); showToast(`Workflow run started${res.job_id ? ` (${res.job_id})` : ''}.`); },
    onError: (err: Error) => showToast(err?.message || 'Failed to run workflow.', 'error'),
  });
  const triggerMut = useMutation({
    mutationFn: () => triggerPost(cfg),
    onSuccess: (res) => { qc.invalidateQueries({ queryKey: ['bot-status'] }); showToast(res.job_id ? `Triggered job ${res.job_id}.` : 'Workflow trigger sent.'); },
    onError: (err: Error) => showToast(err?.message || 'Failed to trigger workflow.', 'error'),
  });

  const isDirty = contentHours !== null || newsHours !== null;

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-5xl mx-auto space-y-6 pb-24">
      <AnimatePresence>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {schedulerRunning ? (
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => pauseMut.mutate()} disabled={pauseMut.isPending || botLoading}
              ...
              {pauseMut.isPending ? <Spinner size={14} /> : <Pause className="w-4 h-4" />} Pause
            </motion.button>
          ) : (
            <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => resumeMut.mutate()} disabled={resumeMut.isPending || botLoading}
              ...
              {resumeMut.isPending ? <Spinner size={14} /> : <Play className="w-4 h-4" />} Resume
            </motion.button>
          )}
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => newsMut.mutate()} disabled={newsMut.isPending}
            ...
            {newsMut.isPending ? <Spinner size={14} /> : <Newspaper className="w-4 h-4" />} Post News
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => engageMut.mutate()} disabled={engageMut.isPending}
            className="bg-brand-elevated border border-brand-border text-brand-text px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider hover:border-brand-accent/40 flex items-center gap-2 disabled:opacity-60">
            {engageMut.isPending ? <Spinner size={14} /> : <MessageSquare className="w-4 h-4" />} Engage
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => runMut.mutate()} disabled={runMut.isPending}
            className="bg-brand-elevated border border-brand-border text-brand-text px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider hover:border-brand-accent/40 flex items-center gap-2 disabled:opacity-60">
            {runMut.isPending ? <Spinner size={14} /> : <GitBranch className="w-4 h-4" />} Run Workflow
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => triggerMut.mutate()} disabled={triggerMut.isPending}
            ...
            {triggerMut.isPending ? <Spinner size={14} /> : <TriangleAlert className="w-4 h-4" />} Trigger
          </motion.button>
          <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => postMut.mutate()} disabled={postMut.isPending}
            ...
            {postMut.isPending ? <Spinner size={14} /> : <Send className="w-4 h-4" />} Post Now
          </motion.button>
        </div>
      </div>
      ...
      {wfList.length > 0 && (
        <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
          <h2 className="text-sm font-bold uppercase tracking-widest text-brand-text-muted mb-4 flex items-center gap-2">
            <GitBranch className="w-4 h-4 text-brand-accent" /> Workflow Definitions
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {wfList.map((wf, i) => (
              <div key={wf.id ?? i} className="bg-brand-elevated border border-brand-border rounded-xl p-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className={cn('w-2 h-2 rounded-full', wf.status === 'running' ? 'bg-brand-success animate-pulse' : wf.enabled ? 'bg-brand-warning' : 'bg-brand-text-muted')} />
                  <span className="text-xs font-bold text-brand-text">{wf.name ?? `Workflow ${i + 1}`}</span>
                </div>
                <div className="text-[9px] font-mono text-brand-text-muted">{wf.status ?? '—'}</div>
                {wf.last_run && <div className="text-[9px] font-mono text-brand-text-muted mt-0.5">Last: {new Date(wf.last_run).toLocaleString()}</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
        ...
        <p className="text-xs text-brand-text-muted font-mono mb-4">Click an hour to toggle it.</p>
        <div className="grid grid-cols-6 md:grid-cols-12 gap-2">
          {ALL_HOURS.map(h => (
            <motion.button key={h} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
              onClick={() => toggleHour(displayContentHours, setContentHours, h)}
              className={cn('py-2 rounded-xl text-xs font-bold font-mono transition-all border',
                displayContentHours.includes(h) ? 'bg-brand-primary/20 border-brand-primary text-brand-primary shadow-glow-primary' : 'bg-brand-elevated border-brand-border text-brand-text-muted hover:border-brand-primary/30')}>
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {displayContentHours.length === 0 && <span className="text-[10px] font-mono text-brand-text-muted">No content hours selected</span>}
        </div>
      </div>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
        ...
        <div className="grid grid-cols-6 md:grid-cols-12 gap-2">
          {ALL_HOURS.map(h => (
            <motion.button key={h} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
              ...
              className={cn('py-2 rounded-xl text-xs font-bold font-mono transition-all border',
                displayNewsHours.includes(h) ? 'bg-brand-warning/20 border-brand-warning text-brand-warning' : 'bg-brand-elevated border-brand-border text-brand-text-muted hover:border-brand-warning/30')}>
              {String(h).padStart(2, '0')}
            </motion.button>
          ))}
        </div>
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {displayNewsHours.length === 0 && <span className="text-[10px] font-mono text-brand-text-muted">No news hours selected</span>}
        </div>
      </div>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
        <h2 className="text-sm font-bold uppercase tracking-widest flex items-center gap-2 mb-5">
          <History className="w-4 h-4 text-brand-text-muted" /> Workflow History
          <span className="ml-auto text-[10px] font-mono text-brand-text-muted">Last 20 runs</span>
        </h2>
        {workflowHistory.length === 0 ? (
          <div className="py-10 text-center text-brand-text-muted font-mono text-xs uppercase">No workflow history available.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-brand-border bg-brand-elevated">
                  {['Run ID', 'Type', 'Status', 'Started', 'Duration'].map(h => (
                    <th key={h} className="py-3 px-4 text-[10px] font-mono uppercase tracking-widest text-brand-text-muted">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {workflowHistory.slice(0, 20).map((run, i) => (
                  <tr key={run.id ?? i} className="border-b border-brand-border/50 hover:bg-brand-elevated/40 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className={cn('px-2 py-0.5 rounded-md text-[10px] font-bold uppercase border font-mono',
                    ...
                    <td className="py-3 px-4 text-xs font-mono text-brand-text-muted">
                      {run.started_at || run.created_at ? new Date((run.started_at || run.created_at) as string).toLocaleString() : '—'}
                    </td>
                    <td className="py-3 px-4 text-xs font-mono text-brand-text-muted">
        ...
        {isDirty && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
            ...
            <div className="bg-brand-surface border border-brand-primary/40 rounded-2xl shadow-2xl p-4 flex items-center gap-4 w-full max-w-md">
              ...
              <button onClick={() => { setContentHours(null); setNewsHours(null); }} className="text-xs font-bold text-brand-text-muted hover:text-brand-text px-3 py-1.5 rounded-lg transition-colors">Discard</button>
              <button onClick={() => saveMut.mutate()} disabled={saveMut.isPending}
                ...
                {saveMut.isPending ? <Spinner size={16} /> : <Check className="w-4 h-4" />} Save Schedule
              </button>
            </div>
          </motion.div>
        )}

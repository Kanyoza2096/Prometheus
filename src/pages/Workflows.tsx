...
type WorkflowNode = { id: string; label: string; icon: any; status: 'active' | 'pending' };
...
const getIcon = (icon: any): React.ComponentType<any> => {
  ...
  if (icon && typeof icon === 'object' && icon.name && ICON_MAP[icon.name]) {
    return ICON_MAP[icon.name];
  }
  if (typeof icon === 'function' || (icon && typeof icon === 'object' && icon.$$typeof)) {
    return icon;
  }
  return Globe;
};
...
export default function Workflows() {
  ...
  const triggerMut = useMutation({
    ...
    onSuccess: (d: any) => {
      const msg = d?.queued ? '✓ Content post queued' : '⚠ Queue full — try again shortly';
      setTriggerToast(msg);
      setTimeout(() => setTriggerToast(null), 3000);
    },
    onError: () => {
      setTriggerToast('✕ Failed to trigger post');
      setTimeout(() => setTriggerToast(null), 3000);
    },
  });
        ...
        <div className="flex items-center gap-3">
          {triggerToast && (
            <motion.span
              ...
              className="text-xs font-mono font-bold text-brand-success"
            >
              {triggerToast}
            </motion.span>
          )}
                  ...
                  >
                    <div className={cn(
                      ...
                      {(() => {
                        ...
                        return <NodeIcon className="w-5 h-5" />;
                      })()}
                    </div>
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-brand-border flex items-center justify-between">
          ...
          <span className="px-2 py-1 bg-brand-bg rounded text-xs font-mono text-brand-text-muted">
            Showing {workflows.length} results
          </span>
        </div>
              ...
              {workflows.map(wf => (
                <tr key={wf.id} className="hover:bg-brand-elevated/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    <button className="p-1.5 text-brand-text-muted hover:text-brand-text transition-colors" title="Edit" onClick={(e) => {
                      e.stopPropagation();
                    ...
                    <button className="p-1.5 text-brand-text-muted hover:text-brand-primary transition-colors" title="Toggle" onClick={(e) => {
                      e.stopPropagation();
                    ...
                    <button className="p-1.5 text-brand-text-muted hover:text-brand-danger transition-colors" title="Delete" onClick={(e) => {
                      e.stopPropagation();

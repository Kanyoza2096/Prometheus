  ...
  return (
    <div className="bg-brand-surface rounded-2xl border border-brand-border flex flex-col h-[400px] transform-gpu">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.map((msg, i) => (
          ...
          >
            <div className={cn(
              ...
              i === 0 && !isStreamPaused ? "bg-brand-primary/5 border-brand-primary/20 shadow-[0_0_15px_rgba(79,70,229,0.1)]" : "hover:bg-brand-elevated/50"
            )}>
        ...
        {messages.length === 0 && (
          <div className="h-full flex items-center justify-center text-brand-text-muted text-sm font-mono opacity-50">
...
export function HealthMatrix() {
  ...
  const overallScore = Math.round(healthMatrix.reduce((acc, curr) => acc + curr.uptime, 0) / healthMatrix.length);
    ...
    <div className="bg-brand-surface rounded-2xl border border-brand-border p-5 flex flex-col h-full min-h-[400px]">
      <div className="flex justify-between items-end mb-6">
        ...
        <div className="text-right">
          <div className="text-3xl font-extrabold font-mono text-brand-success">{overallScore}%</div>
          <div className="text-[10px] uppercase tracking-widest text-brand-success/70">Overall Health</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 flex-1">
        {healthMatrix.map(service => (
          <motion.div 
            ...
            className="h-full z-0 relative transform-gpu"
          >
            <div className={cn(
...
export function RevenueTicker() {
  ...
  return (
    <div className="bg-brand-surface rounded-2xl border border-brand-border p-5 flex flex-col justify-between relative overflow-hidden group">
      <div className="absolute top-0 right-0 w-32 h-32 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-brand-success/20 to-transparent rounded-full -translate-y-1/2 translate-x-1/2"></div>
      <div>
  ...
  return (
    <div className="bg-brand-surface rounded-2xl border border-brand-border p-5 flex flex-col col-span-1 lg:col-span-2">
      <div className="flex justify-between items-center mb-4">
        ...
        <button className="text-xs bg-brand-primary/10 text-brand-primary px-3 py-1.5 rounded-lg font-bold uppercase hover:bg-brand-primary/20 transition-colors flex items-center">
      ...
      <div className="flex space-x-4 overflow-x-auto pb-2 custom-scrollbar">
        {recentPosts.map(post => (
          <motion.div 
            key={post.id}
            whileHover={{ y: -2 }}
            className="flex-shrink-0 w-64 z-0 relative"
          >
            <div className="bg-brand-bg rounded-xl border border-brand-border overflow-hidden cursor-pointer group h-full">
...
export function GuardianAlertsWidget() {
  ...
  return (
    <div className={cn(
      ...
      {hasCritical && <div className="absolute top-0 left-0 w-full h-1 bg-brand-danger animate-pulse"></div>}
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-2">
        {guardianAlerts.map(alert => (
          <div key={alert.id} className="p-3 bg-brand-bg rounded-xl border border-brand-border flex items-start space-x-3">
            <div className={cn(
